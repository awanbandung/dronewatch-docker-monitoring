...
export default function StreamingPage() {
  ...
  const [flightSec, setFlightSec]         = useState(selectedDrone.flightSec)
  ...
  const telem = useTelemetry(selectedDrone)

  // Flight timer
  useEffect(() => {
    setFlightSec(selectedDrone.flightSec)
  ...
  // Auto-expand grid when operator starts searching or filtering
  useEffect(() => {
    if (searchQuery || filterOpen) setGridOpen(true)
  }, [searchQuery, filterOpen])

  // Cleanup stopConfirm timer on unmount
  ...
  function handleSelectDrone(drone) {
  ...
  const recLabel = recording
    ? `${padZ(Math.floor(recSec / 60))}:${padZ(recSec % 60)}`
    : 'RECORD'
  ...
  const hue = 100 + (parseInt(selectedDrone.id.replace('DRN-','')) * 3) % 40
  ...
  return (
    <div className="fixed inset-0 flex flex-col" style={{ background: '#0b0e14' }}>
      ...
      <div className="fixed left-0 right-0 flex flex-col" style={{ top: 42, bottom: 28 }}>
        ...
        {/* Top section: featured feed + right col */}
        <div className="flex flex-1 min-h-0">
          ...
          <div className="flex-1 relative overflow-hidden"
            ...
            <div className="absolute top-0 left-0 right-0 z-20 flex items-center gap-2 px-3"
              ...
              <span className="mono text-[11px] text-[#dde4ef]">{selectedDrone.area}</span>
              <span className="mono text-[10px] text-[#6b7b90]">/</span>
              <span className="mono text-[11px] tracking-[1px]" style={{ color: 'var(--accent)' }}>
                Drone {selectedDrone.droneNum}
              </span>
            </div>
            ...
            <DroneCanvas
              hue={hue}
              noisy={selectedDrone.status === 'red'}
              inactive={selectedDrone.status === 'inactive'}
              style={{ position: 'absolute', inset: 0, width: '100%', height: '100%' }}
            />

            {/* HUD — dims when operator isn't on the feed, keeping video dominant */}
              ...
              <div className="absolute flex flex-col gap-1" style={{ top: 36, left: 10 }}>
                ...
                <HudBox label="STATUS">
                  <span className="mono text-[11px] tracking-[1px]"
                        style={{ color: STATUS_COLORS[selectedDrone.status] }}>
                    {telem?.status ?? selectedDrone.flightStatus}
                  </span>
                </HudBox>
              </div>
                ...
                <div className="flex items-end gap-3">
                  {[
                    ...
                    <div key={i} className="flex flex-col items-end">
                      ...
                      <span className="mono text-[8px] tracking-[2px] mt-0.5" style={{ color: r.color === '#dde4ef' ? '#6b7b90' : r.color }}>
                        {r.label}
                      </span>
                    </div>
                  ))}
          ...
          {/* Right Column */}
          <div className="w-[280px] shrink-0 flex flex-col overflow-hidden" style={{ background: '#0f1319' }}>
            ...
            <div className="sec-hd flex-shrink-0">
              ...
              <span className="mono text-[8px] px-1.5 py-0.5 rounded-sm" style={{ border: '1px solid rgba(255,255,255,0.10)', color: '#6b7b90' }}>
            ...
            <div className="flex-1 relative min-h-0 overflow-hidden" style={{ background: '#0c1a0c' }}>
              <MiniMap telem={telem} drone={selectedDrone}/>
            </div>
            ...
            <div className="flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,0.10)' }}>
              <div className="sec-hd">
                ...
                <span className="status-badge">{selectedDrone.id}</span>
              </div>
              <div className="px-3 py-2 flex flex-col gap-0.5">
                {[
                  { key: 'Serial Number',      val: selectedDrone.serial,      color: 'var(--accent)' },
                  { key: 'Operator ID',         val: selectedDrone.operatorId  },
                  { key: 'Uptime',              val: formatUptime(flightSec)   },
                  { key: 'Status Penerbangan',  val: STATUS_LABELS[selectedDrone.status], color: STATUS_COLORS[selectedDrone.status] },
                  { key: 'Type',                val: selectedDrone.type        },
                  { key: 'Area',                val: selectedDrone.area        },
                ].map(row => (
        ...
        <div className="flex-shrink-0" style={{ borderTop: '1px solid rgba(255,255,255,0.10)', background: '#0f1319' }}>
          ...
          <div className="flex items-center gap-3 px-3 py-1.5"
            ...
            {/* Collapsed: fleet status summary */}
            {!gridOpen && (
            ...
            {/* Expanded: search + filter + sort controls */}
            {gridOpen && (
              <>
...
function MiniMap({ telem, drone }) {
  // Drone position normalized to SVG coords
